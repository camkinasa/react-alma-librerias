...
const Cart = () =>{
    ...
    const createOrder = () => {
      ...
      let order = {
        buyer: {
          name: "Camila Ramirez",
          email: "camramirez0594@gmail.com",
          phone: "123456789"
        },
      ...
      createOrderInFirestore()
        .then(result => {
          alert('Se realizó tu orden de compra con el número  ' + result.id + '\n\n')
          compra.cart.forEach(async (libro) => {
            const bookRef = doc(DB, "products", libro.id);
            await updateDoc(bookRef, {
              stock: increment(-libro.quantity)
            });
          });
          compra.clear();
        })
                ...
                {compra.cart.map((book) => (
                  <Row key={book.id} className="mb-1">
                    ...
                    <Col m={2}>
                      <Button
                        className="btn btn-danger"
                        onClick={() => compra.removeBook(book)}
                      >
